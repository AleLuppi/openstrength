...
import { calculatePercentage1RM, calculateRepsFromTable, calculateRpeFromTable, estimateMissingLineProps } from "@/helpers/charts/chartDatasetComputations";
...
describe("Test on calculations for line estimation", ()=>{
  test("TEST LINE 2 ", () => {
    ...
    const lineResult = estimateMissingLineProps(lineToTest, Number(maxlift.value));
  ...
  test("TEST LINE 7 ", () => {
    ...
    if(lineResult?.loadSupposedValue){
      expect((100*lineResult?.loadSupposedValue/Number(maxlift.value)).toFixed(2)).toBe("81.67")
    }
  ...
  test("TEST LINE 9 ", () => {
    ...
    expect(lineResult?.repsValue).toBe(undefined);
    if (lineResult?.loadValue){
      expect((100*lineResult?.loadValue/Number(maxlift.value)).toFixed(2)).toBe("83.33")
    }
